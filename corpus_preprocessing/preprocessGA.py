...
def remove_extra_rows_GA(state = "GA"):
    state = "GA"
    file_path = f'../../data/csv_files/{state}_disciplinary_actions.csv'
    df = pd.read_csv(file_path)

    # Keep only: 'misconduct' column
    cols_to_keep = ['Descriptions'] 
    df = df[cols_to_keep]

    # Apply the function to each description and create a new 'labels' column
    df['labels'] = df['Descriptions'].apply(find_matches)
    ...
    labels_ga = pd.DataFrame(df_filtered["labels"].to_list())

    df_filtered  = pd.concat([df_filtered["Descriptions"].reset_index(drop=True), labels_ga.reset_index(drop=True)], axis=1)
